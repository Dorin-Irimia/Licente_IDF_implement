...
extern int g_bPOSTRequestInProcess;
...
{ .u8Temperature = 27, .u8Humidity = 50, .u8Comfort = 24, .bIsLocked = true,
		.bIsOccupied = false , .bButtonHonk = false, .bButtonFindMyCar = false};
...
void COM_vProcessPostRequest(void)
{
	...
	if (g_bPOSTRequestInProcess)
	{
		...
		{
			g_POST_DataStructure.bButtonFindMyCar ++;
		}

		else if (strcmp(g_cPOSTBuffer, "button=Honk") == 0)
		{
			g_POST_DataStructure.bButtonHonk ++;
		}
